...
CREATE TABLE VACANCIES (ID INTEGER GENERATED BY DEFAULT AS IDENTITY (START WITH 1) PRIMARY KEY NOT NULL, STORE_ID INTEGER, SECTION_ID INTEGER, NUMBERS INTEGER, ENTRY_DATE DATE)
ALTER TABLE VACANCIES ADD CONSTRAINT FK_VACANCIES_STORE_ID FOREIGN KEY (STORE_ID) REFERENCES STORE (ID)
CREATE TABLE TRANSFERS (ID INTEGER GENERATED BY DEFAULT AS IDENTITY (START WITH 1) PRIMARY KEY NOT NULL), VACANCY_ID INTEGER, EMPLOYEE_ID INTEGER, SCORE INTEGER, IS_PROCESSED BOOLEAN)
ALTER TABLE TRANSFERS ADD CONSTRAINT FK_TRANSFERS_VACANCY_ID FOREIGN KEY (VACANCY_ID) REFERENCES VACANCIES (ID)
ALTER TABLE TRANSFERS ADD CONSTRAINT FK_TRANSFERS_VACANCY_ID FOREIGN KEY (EMPLOYEE_ID) REFERENCES EMPLOYEE (ID)